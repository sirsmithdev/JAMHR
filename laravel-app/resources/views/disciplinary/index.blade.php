<x-app-layout>
    ...
    <div class="space-y-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-serif text-foreground">Disciplinary Actions</h1>
                <p class="text-muted-foreground mt-1">Manage employee discipline, warnings, and performance improvement plans.</p>
            </div>
            <a href="{{ route('disciplinary.create') }}" class="inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-primary hover:bg-primary/90 shadow-lg transition-colors">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4">
                ...
                <div class="text-xs text-muted-foreground">Open Cases</div>
            </div>
            <div class="bg-white rounded-lg shadow-sm p-4">
                <div class="text-2xl font-bold text-foreground">{{ $stats['this_month'] }}</div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4">
                <div class="text-2xl font-bold text-blue-600">{{ $stats['pips_active'] }}</div>
                <div class="text-xs text-muted-foreground">Active PIPs</div>
            </div>
            <div class="bg-white rounded-lg shadow-sm p-4">
                <div class="text-2xl font-bold text-red-600">{{ $stats['suspensions'] }}</div>
        ...
        <!-- Filters -->
        <div class="bg-white rounded-lg shadow-sm p-4">
            <form action="{{ route('disciplinary.index') }}" method="GET" class="flex flex-wrap gap-4">
                ...
                <select name="employee_id" class="px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                ...
                <select name="type" class="px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    ...
                    @foreach(['Verbal Warning', 'Written Warning', 'Final Written Warning', 'Suspension', 'Performance Improvement Plan'] as $type)
                ...
                <select name="status" class="px-3 py-2 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    ...
                    @foreach(['Open', 'Under Review', 'Resolved', 'Appealed'] as $status)
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-muted/30">
                        <tr>
                            ...
                            <th class="text-left px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Incident Date</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">Acknowledged</th>
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        <tr class="hover:bg-muted/5">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-medium">
                                        {{ $action->employee->initials }}
                                    </div>
                                    <div>
                                        <a href="{{ route('disciplinary.show', $action) }}" class="font-medium text-foreground hover:text-primary">
                                            {{ $action->employee->full_name }}
                                        </a>
                                        <div class="text-sm text-muted-foreground">{{ $action->employee->department }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-sm">{{ $action->incident_date->format('M d, Y') }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                <svg class="h-5 w-5 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('disciplinary.show', $action) }}" class="text-muted-foreground hover:text-foreground" title="View">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                    </a>
                                    <a href="{{ route('disciplinary.edit', $action) }}" class="text-muted-foreground hover:text-foreground" title="Edit">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            @if($disciplinaryActions->hasPages())
